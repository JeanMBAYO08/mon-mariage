...
export async function upsertGuest(payload, { defaultStatut }) {
  ...
  const existing = invites.find((g) => String(g.nom || "").trim().toLowerCase() === nom.toLowerCase());
  if (existing) {
    if (existing.statut === "entree") {
      return { ok: true, updated: true, alreadyIn: true, guest: existing };
    }
    ...
    return { ok: true, updated: true, guest: existing };
  }
  ...
  invites.push(guest);
  await saveInvites(invites);
  return { ok: true, created: true, guest };
}
...
export async function updateGuest(payload) {
  ...
  await saveInvites(invites);
  return { ok: true, updated: true, guest };
}
...
export function toCsv(invites) {
  const header = "code,nom,type,personnes,table,whatsapp,statut,date_entree,notes";
  const rows = invites.map((g) =>
    [g.code, g.nom, g.type, g.personnes, g.table, g.whatsapp, g.statut, g.date_entree, g.notes]
