...
const RSVP_DEADLINE = process.env.RSVP_DEADLINE || "2026-09-10T23:59:59+01:00";
const RSVP_MANUAL_ONLY = String(process.env.RSVP_MANUAL_ONLY || "0") === "1";

export function isRsvpOpen(now = new Date()) {
  if (RSVP_MANUAL_ONLY) return false;
  const deadline = new Date(RSVP_DEADLINE);
  if (Number.isNaN(deadline.getTime())) return now <= new Date("2026-09-10T23:59:59+01:00");
