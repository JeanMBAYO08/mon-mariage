...
const RENDER_API_URL = (
  process.env.RENDER_API_URL || "https://mon-mariage-f7dv.onrender.com"
).replace(/\/$/, "");
...
function mergeByCode(...lists) {
  ...
  lists.flat().forEach((guest) => {
    if (!guest) return;
    const code = String(guest.code || "").trim().toUpperCase();
    if (!code) return;
    const prev = map.get(code) || {};
    map.set(code, normalizeGuestRecord({ ...prev, ...guest, code }));
  });
  return [...map.values()];
}
...
async function loadBlobInvites() {
  ...
  try {
    ...
    // [] vide = pas encore migré, on ne l’utilise pas comme source unique
...
async function loadRenderInvites() {
  try {
    const res = await fetch(`${RENDER_API_URL}/api/invites`, {
      headers: { Accept: "application/json" },
    });
    if (!res.ok) return null;
    const data = await res.json();
    if (!data?.ok || !Array.isArray(data.guests)) return null;
    return data.guests.map(normalizeGuestRecord);
  } catch (err) {
    console.warn("Render sync:", err);
    return null;
  }
}

export async function loadInvites() {
  if (Array.isArray(globalThis.__PJ_INVITES__) && globalThis.__PJ_INVITES__.length) {
    return globalThis.__PJ_INVITES__;
  }

  const bundled = loadBundledInvites();
  const [blob, render] = await Promise.all([loadBlobInvites(), loadRenderInvites()]);

  // Fusion : bundled + Render (historique) + Blob (modifs Vercel si présentes)
  const merged = mergeByCode(bundled, render || [], blob || []);
  globalThis.__PJ_INVITES__ = merged;

  // Persiste la fusion dans Blob dès que possible
  if (merged.length && process.env.BLOB_READ_WRITE_TOKEN) {
    try {
