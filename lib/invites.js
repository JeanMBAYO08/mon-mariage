...
async function loadGitHubInvites() {
  try {
    ...
    if (!res.ok) {
      console.warn("GitHub load status:", res.status);
      return null;
    }
...
async function saveGitHubInvites(invites) {
  ...
  if (!token) {
    throw new Error(
      "GITHUB_TOKEN manquant sur Vercel — les suppressions ne peuvent pas être enregistrées."
    );
  }
  ...
  if (!metaRes.ok) {
    throw new Error(`Lecture GitHub impossible (${metaRes.status})`);
  }
  const meta = await metaRes.json();
  const sha = meta.sha;
  if (!sha) throw new Error("SHA GitHub introuvable pour data/invites.json");
  ...
  if (!putRes.ok) {
    ...
    throw new Error(`Écriture GitHub impossible (${putRes.status}): ${detail.slice(0, 200)}`);
  }
}
...
export async function loadInvites() {
  ...
  // 1) GitHub (source de vérité en prod Vercel)
...
export async function saveInvites(invites) {
  ...
  if (!onVercel) {
    ...
    await saveBlobInvites(invites);
    return;
  }

  // Prod Vercel : GitHub obligatoire pour survivre au refresh
  await saveGitHubInvites(invites);
  // Best-effort Blob
  await saveBlobInvites(invites);
}
