...
async function loadBlobInvites() {
  try {
    ...
    const { blobs } = await list({ prefix: "invites/", limit: 20, token });
...
export async function loadInvites() {
  // Toujours recharger depuis le stockage durable (évite liste figée à 9)
  // sauf au sein de la même requête serverless via un flag court.
  if (Array.isArray(globalThis.__PJ_INVITES__) && globalThis.__PJ_INVITES_FRESH__) {
  ...
  if (fromBlob) {
    ...
    globalThis.__PJ_INVITES_FRESH__ = true;
    return fromBlob;
  }
