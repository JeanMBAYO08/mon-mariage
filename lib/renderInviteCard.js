...
function confirmedTableLabel(guest) {
  const tableRaw = String(guest?.table || "").trim();
  if (!tableRaw) return "";
  if (/^(sans table|à confirmer|a confirmer|-)$/i.test(tableRaw)) return "";
...
export function inviteCardLabels(guest) {
  ...
  return { nom, type, table: confirmedTableLabel(guest) };
}
...
export async function renderInviteCard(guest) {
  ...
  const photo = await sharp(readFileSync(BASE_CARD))
    .rotate()
    .resize(w, h, { kernel: "lanczos3", withoutEnlargement: true })
    .toBuffer();
  ...
  const showTable = Boolean(table);
  const blockH =
    nameLines.length * Math.round(nameSize * 1.2) +
    lineGap +
    typeSize +
    (showTable ? lineGap + tableSize : 0);
